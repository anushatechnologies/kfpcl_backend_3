...
@Repository("buyerProductRepository")
public interface ProductRepository extends JpaRepository<Product, Long> {
    ...
    @Query("SELECT p FROM BuyerProduct p WHERE p.isActive = true AND " +
    ...
    @Query("SELECT p FROM BuyerProduct p LEFT JOIN ContactLead l ON p.id = l.product.id WHERE p.isActive = true GROUP BY p.id, p.name, p.brand, p.category, p.subcategory, p.supplier, p.mainImageUrl, p.minOrderQuantity, p.indicativePrice, p.numericPrice, p.specifications, p.isActive, p.createdAt ORDER BY COUNT(l) DESC, p.id DESC")
    List<Product> findTrendingProducts(Pageable pageable);

    @Query("SELECT p FROM BuyerProduct p WHERE p.isActive = true ORDER BY p.id DESC")
